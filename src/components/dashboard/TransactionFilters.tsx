...
export function TransactionFilters({ filters, onChange }: TransactionFiltersProps) {
  const hasFilters = Boolean(filters.search || filters.kind !== "all" || filters.category || filters.date || filters.currency);
  ...
  return (
    <div className="filters" aria-label="Filtros de movimientos">
      ...
      <label><span className="sr-only">Filtrar por moneda</span><select value={filters.currency} onChange={(event) => update("currency", event.target.value as Filters["currency"])}><option value="">Todas las monedas</option><option value="CLP">CLP</option><option value="USD">USD</option></select></label>
      ...
      {hasFilters && <button type="button" onClick={() => onChange({ search: "", kind: "all", category: "", date: "", currency: "" })}>Limpiar</button>}
    </div>
  );
}
